...
```
ha-flask-project/
├── .gitignore               # Ignores .tfstate files, .pem keys, and inventory.ini
├── main.tf                  # Main Terraform configuration (VPC, Subnets, EC2, IGW, SG)
├── ansible/
│   ├── inventory.tmpl       # Terraform template to generate the host list
│   ├── inventory.ini        # (Generated by Terraform) Ansible's list of live host IPs
│   ├── run_deployment.yml   # Master Ansible playbook, which calls the webserver role
│   └── roles/
│       └── webserver/
│           ├── tasks/
│           │   └── main.yml # Task logic (Install Python, Copy File, Start App)
│           └── files/
│               └── adventure_game.py # The Flask V2 game code
